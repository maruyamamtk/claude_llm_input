...
### Gmail OAuth2 認証時に `403: access_denied` が出る

OAuth同意画面のアプリが「テスト中」ステータスのため、テストユーザーとして登録されていないアカウントはアクセスを拒否されます。

**対処方法:**

1. [GCPコンソール](https://console.cloud.google.com/) → **「APIs & Services」** → **「OAuth consent screen」** を開く
2. 画面上部のタブから **「オーディエンス」** をクリック
   - ※ウィザード形式ではなくタブ形式のUIが表示される場合はこちら
3. 「テストユーザー」セクションの **「+ ADD USERS」** をクリック
4. `marumaru5922@gmail.com` を入力して **「追加」** → **「保存」**
5. 再度 [Step 5: token.json を生成する](#step-5token-json-を生成する) を実行
